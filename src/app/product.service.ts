import { HttpClient, HttpErrorResponse, HttpParams } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { Observable, catchError, throwError } from 'rxjs';
import { ViewtableComponent } from './viewtable/viewtable.component';

@Injectable({
  providedIn: 'root'
})
export class ProductService {
constructor(private http:HttpClient) { }

productList(): Observable<any[]> {
  
  return this.http.get<any[]>('http://localhost:5000/api/products/');
}



deleteProduct(productId: string): Observable<any> {
  const url = `http://localhost:5000/api/products/delete?id=${productId}`;
  return this.http.delete<any>(url);
}


users(): Observable<any[]> {

  return this.http.get<any[]>('http://localhost:5000/api/user/allusers');
}


  addProduct(data:any): Observable<any[]> {
 
    return this.http.post<any>("http://localhost:5000/api/products/add",data);
  }

  addBrand(data:any): Observable<any[]> {
 
    return this.http.post<any>("http://localhost:5000/api/brand/",data);
  }


  allBrand(): Observable<any[]> {
 
    return this.http.get<any>("http://localhost:5000/api/brand/view");
  }

  addCategory(data:any): Observable<any[]> {
 
    return this.http.post<any>("http://localhost:5000/api/category/",data);
  }

  allcategory(): Observable<any[]> {
 
    return this.http.get<any>("http://localhost:5000/api/category/view");
  }

  deleteCategory(id: number): Observable<void> {
    return this.http.delete<void>(`http://localhost:5000/api/category/delete/${id}`
    );
  }



  addSubcategory(data:any): Observable<any[]> {
 
    return this.http.post<any>("http://localhost:5000/api/subCategory/",data);
  }
  allSubcategory(): Observable<any[]> {
 
    return this.http.get<any>("http://localhost:5000/api/subCategory/view");
  }


  addSection(data:any): Observable<any[]> {
 
    return this.http.post<any>("http://localhost:5000/api/section/",data);
  }

  allSection(): Observable<any[]> {
 
    return this.http.get<any>("http://localhost:5000/api/section/view");
  }


  
  updateProduct(productId: string, updatedData: any): Observable<any> {
    const url = `http://localhost:5000/api/products/update?id=${productId}`;
    return this.http.put<any>(url, updatedData);
  }
  


getOrders(): Observable<any[]> {
return this.http.get<any>(`http://localhost:5000/api/orders/order`);
}

}
